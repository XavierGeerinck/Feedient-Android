...
public class Instagram implements IProviderModel, IOAuth2Provider {
    ...
    private void _doActionLike(final FeedPost feedPost) {
        feedientService.doActionInstagramLike(accessToken, feedPost.getProvider().getId(), "like", feedPost.getId(), new Callback<PerformAction>() {
            @Override
    ...
    private void _doActionUnlike(final FeedPost feedPost) {
        feedientService.undoActionInstagramLike(accessToken, feedPost.getProvider().getId(), "unlike", feedPost.getId(), new Callback<PerformAction>() {
            @Override
            public void success(PerformAction performAction, Response response) {
